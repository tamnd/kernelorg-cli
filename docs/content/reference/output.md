...
describes what users get. Pick a format with `-o`, or let kernel choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
kernel <command> -o table   # aligned columns for reading
kernel <command> -o jsonl   # one JSON object per line, for piping
kernel <command> -o json    # a single JSON array
kernel <command> -o csv     # spreadsheet friendly
kernel <command> -o tsv     # tab-separated
kernel <command> -o url     # just the URL column
kernel <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
kernel <command> --fields id,title,url
```
...
```bash
kernel <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
kernel <command>            # a table, because this is a terminal
kernel <command> | wc -l    # JSONL, because this is a pipe
```
